...
print('contranst config = {}'.format(factor))
...
for i in range(b_img.size[0]):
    for j in range(b_img.size[1]):
        #print(pixelsNew[i,j])
        if lo_lim <= b_pixelsNew[i,j] < hi_lim:
            b_pixelsNew[i,j] = black
        else:
            b_pixelsNew[i,j] = white
...
for i in range(c_img.size[0]):
    for j in range(c_img.size[1]):
        #print(pixelsNew[i,j])
        if c_pixelsNew[i,j] >= hi_lim:
            c_pixelsNew[i,j] = black
        else:
